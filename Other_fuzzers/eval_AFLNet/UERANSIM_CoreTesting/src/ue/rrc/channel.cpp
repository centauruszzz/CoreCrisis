...
void UeRrcTask::sendRrcMessage(int cellId, ASN_RRC_UL_CCCH_Message *msg)
{
    ...
    fprintf(stderr, "[RRC-TX] UL-CCCH (%d bytes): ", pdu.length());
    for (int i = 0; i < pdu.length(); i++)
        fprintf(stderr, "%02x", pdu.data()[i]);
    fprintf(stderr, "\n");
...
void UeRrcTask::sendRrcMessage(int cellId, ASN_RRC_UL_CCCH1_Message *msg)
{
    ...
    fprintf(stderr, "[RRC-TX] UL-CCCH1 (%d bytes): ", pdu.length());
    for (int i = 0; i < pdu.length(); i++)
        fprintf(stderr, "%02x", pdu.data()[i]);
    fprintf(stderr, "\n");
...
void UeRrcTask::sendRrcMessage(ASN_RRC_UL_DCCH_Message *msg)
{
    ...
    fprintf(stderr, "[RRC-TX] UL-DCCH (%d bytes): ", pdu.length());
    for (int i = 0; i < pdu.length(); i++)
        fprintf(stderr, "%02x", pdu.data()[i]);
    fprintf(stderr, "\n");
