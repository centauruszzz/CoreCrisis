...
region_t* extract_requests_5gc(unsigned char* buf,
                               ...
                               unsigned int* region_count_ref)
{
  ...
  while (byte_count < buf_size) {
    ...
    // If we have read 0x0a, we have gone through a request/region and 
    // we can save the resion info if it's not empty. Otherwise, continue processing input.
    if (memcmp(&mem[mem_count], terminator, 1) == 0 && cur_start != cur_end) {
      region_count++;
      regions = (region_t *)ck_realloc(regions, region_count * sizeof (region_t));
      regions[region_count - 1].start_byte = cur_start;
      regions[region_count - 1].end_byte = cur_end - 1;
      regions[region_count - 1].state_sequence = NULL;
      regions[region_count - 1].state_count = 0;
      ...
      // Create the last region if it's not empty and exit
      if (cur_end == buf_size - 1) {
      ...
      // we have reached a region longer than mem buffer size (initially 1024)
      // so we increase the mem buffer size.
      if (mem_count == mem_size) {
        // WARN integer overflow is possible
        mem_size += 1024;
        mem = (char *)ck_realloc(mem, mem_size);
      }
    }
...
{
  if (getenv("AFLNET_DEBUG")) {
    char *tmp = (char *)ck_alloc(buf_size + 1);
    memcpy(tmp, buf, buf_size);
    tmp[buf_size] = '\0';
    ACTF("%s buf=\"%s\"", __func__, tmp);
  }
  if (!strncmp(buf, "UNKNOWN", buf_size)) {
    return (unsigned int)UNKNOWN;
  } else if (!strncmp(buf, "registrationAccept", buf_size)) {
    return (unsigned int)registrationAccept;
  } else if (!strncmp(buf, "registrationReject", buf_size)) {
    return (unsigned int)registrationReject;
  } else if (!strncmp(buf, "deregistrationAccept", buf_size)) {
    return (unsigned int)deregistrationAccept;
  } else if (!strncmp(buf, "deregistrationRequest", buf_size)) {
    return (unsigned int)deregistrationRequest;
  } else if (!strncmp(buf, "serviceReject", buf_size)) {
    return (unsigned int)serviceReject;
  } else if (!strncmp(buf, "serviceAccept", buf_size)) {
    return (unsigned int)serviceAccept;
  } else if (!strncmp(buf, "configurationUpdateCommand", buf_size)) {
    return (unsigned int)configurationUpdateCommand;
  } else if (!strncmp(buf, "authenticationRequest", buf_size)) {
    return (unsigned int)authenticationRequest;
  } else if (!strncmp(buf, "authenticationReject", buf_size)) {
    return (unsigned int)authenticationReject;
  } else if (!strncmp(buf, "authenticationResult", buf_size)) {
    return (unsigned int)authenticationResult;
  } else if (!strncmp(buf, "identityRequest", buf_size)) {
    return (unsigned int)identityRequest;
  } else if (!strncmp(buf, "securityModeCommand", buf_size)) {
    return (unsigned int)securityModeCommand;
  } else if (!strncmp(buf, "gmmStatus", buf_size)) {
...
unsigned int* extract_response_codes_5gc(unsigned char* buf,
                                         unsigned int buf_size,
                                         unsigned int* state_count_ref)
{
  if (getenv("AFLNET_DEBUG"))
    ACTF("%s enter buf_size=%d, buf=\"%s\"\n", __func__, buf_size, buf);
  char *mem;
  unsigned int byte_count = 0;
  unsigned int mem_count = 0;
  unsigned int mem_size = 1024;
  ...
  while (byte_count < buf_size) {
    ...
    if (memcmp(&mem[mem_count], terminator, 1) == 0) {
      // Extract the response code which is the first 3 bytes
      // hexdump("Response", mem, 0, mem_count);
      if (strncmp("DONE", mem, mem_count) != 0) {
        unsigned int message_code = message_code_5gc(mem, mem_count);
        if (getenv("AFLNET_DEBUG")) ACTF("%s message_code=%d", __func__, message_code);
...
{
  if (getenv("AFLNET_DEBUG")) ACTF("%s fname=%s, region_count=%d", __func__, fname, region_count);
  FILE *fseed = NULL;
...
int net_send(int sockfd, struct timeval timeout, char *mem, unsigned int len) {
  if (getenv("AFLNET_DEBUG")) {
    char * t = (char *) ck_alloc(len + 1);
    memcpy(t, mem, len);
    t[len] = '\0';
    ACTF("%s sockfd=%d, len=%d, mem=\"%s\"", __func__, sockfd, len, t);
  }
  unsigned int byte_count = 0;
  int n;
  struct pollfd pfd[1];
...
int net_recv(int sockfd, struct timeval timeout, int poll_w, char **response_buf, unsigned int *len) {
  if (getenv("AFLNET_DEBUG")) ACTF("%s enter sockfd=%d, poll_w=%d", __func__, sockfd, poll_w);
  char temp_buf[1000];
  int n;
  struct pollfd pfd[1];
  pfd[0].fd = sockfd;
  ...
  if (getenv("AFLNET_DEBUG")) ACTF("%s poll rv=%d", __func__, rv);
  ...
  if (rv > 0) {
    if (pfd[0].revents & POLLIN) {
      ...
      if ((n < 0) && (errno != EAGAIN)) {
        if (getenv("AFLNET_DEBUG")) ACTF("%s:%d net_recv failed n=%d, errno=%s", 
            __func__, __LINE__, n, strerror(errno));
        return 1;
      }
      while (n > 0) {
        usleep(10);
        ...
        *len = *len + n;
        ...
        if ((n < 0) && (errno != EAGAIN)) {
          if (getenv("AFLNET_DEBUG"))
            ACTF("%s:%d net_recv failed n=%d, errno=%s", __func__, __LINE__, n, strerror(errno));
          return 1;
        }
      }
    }
    ...
    if (rv < 0) { // an error was returned
      if (getenv("AFLNET_DEBUG")) ACTF("%s:%d net_recv failed", __func__, __LINE__);
      return 1;
    }
  ...
  if (getenv("AFLNET_DEBUG")) {
    if (*len > 0)
      ACTF("%s len=%d, response_buf=\"%s\"", __func__, *len, *response_buf);
    else
      ACTF("%s len=%d", __func__, *len);
  }
  return 0;
}
